...
Since the requirement is Public IP (for testing):

Under Networking Type, select:
Public
Leave DNS name label blank unless specifically required.
Ensure:
Public IP = Enabled
Port = 80
...
The lab instructions appear to combine two deployment models:

Model A - VNet Deployment
Deploy into Europe WebSubnet
Gets private IP (like 172.16.1.4)
No public IP
Model B - Public Deployment
Deploy with Public networking
Gets public IP
Not attached to WebSubnet

Azure Container Instances typically use one model or the other.
